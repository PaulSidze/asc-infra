...
locals {
  ...
  vpc_id = "${element(concat(aws_vpc_ipv4_cidr_block_association.asc_vpc.*.vpc_id, aws_vpc.asc_vpc.*.id, list("")), 0)}"
}
...
resource "aws_vpc_ipv4_cidr_block_association" "asc_vpc" {
...
# DHCP Options Set
###################
resource "aws_vpc_dhcp_options" "asc_vpc" {
...
###############################
resource "aws_vpc_dhcp_options_association" "aws_vpc" {
  count = "${var.create_vpc && var.enable_dhcp_options ? 1 : 0}"

  vpc_id          = "${local.vpc_id}"
  dhcp_options_id = "${aws_vpc_dhcp_options.asc_vpc.id}"
}
...
resource "aws_nat_gateway" "asc_vpc" {
